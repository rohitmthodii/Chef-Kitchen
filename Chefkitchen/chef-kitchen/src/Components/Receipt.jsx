...
const ReceiptPopup = ({ isOpen, onClose, cart, orderDateTime, orderType }) => {
      ...
      <div className="bg-[#1F1D2B] w-[90%] max-w-md rounded-xl p-6 text-white relative">
        ...
        <div className="space-y-2 mt-4 max-h-[300px] overflow-y-auto">
          {cart.map((item) => (
            ...
            >
              <p className="max-w-60 truncate w-full">{item.name}</p>
              <p className="max-w-20 w-full text-gray-300">
        ...
        <button className="mt-5 w-full bg-[#F99147] py-2 rounded-lg hover:bg-[#f77b22] shadow-2xl">
          Confirm Order
        </button>
        <button
          onClick={onClose}
          className="mt-2 w-full py-2 rounded-lg border hover:bg-[#22202e] border-gray-500/50 shadow-2xl">
          Close
        </button>
      </div>
    </div>
